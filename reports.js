...
export default async function handler(req, res){
  try{
    ...
    let out = items.filter(Boolean).map(r=>({ id:r.id, store:r.store, type:r.type, submittedBy:r.submittedBy, date:r.date, result:r.result, color:r.color, submittedAt:r.submittedAt, mode:r.mode, spotSections:r.spotSections, spotTitles:r.spotTitles }));
