    ...
    recs.filter(Boolean).filter(r => !r.deleted).forEach(r => {
      (r.items || []).forEach(it => {
        ...
        out.push({
          ...
          materials: it.materials || '', itemStatus: istat, redoReason: it.redoReason || '', secondLook: it.secondLook || '', reviewingSince: (it.reviewClaim && it.reviewClaim.at) || '', submittedForReviewAt: it.submittedForReviewAt || '', blockedReason: it.blockedReason || '', log: Array.isArray(it.log) ? it.log : [],
