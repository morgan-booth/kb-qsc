...
export default async function handler(req, res) {
  try {
    ...
    for (const store of Object.keys(byStore)) {
      const items = byStore[store].sort((a, b) => b.days - a.days);
      const lines = items.map(x => '• ' + x.section + ' — ' + x.item + ' (' + x.days + 'd overdue)');
      const text = '🔴 *' + store + ' — ' + items.length + ' overdue punch-list item' + (items.length > 1 ? 's' : '') + '*\n' + lines.join('\n') + '\n' + base + '/kb-qsc-punchlist.html?store=' + encodeURIComponent(store);
